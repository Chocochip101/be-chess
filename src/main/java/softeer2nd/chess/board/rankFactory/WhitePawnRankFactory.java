...
public class WhitePawnRankFactory implements RankFactory {
    ...
    public List<Piece> createPieces(int rowIndex) {
        IntStream.range(0, COL_COUNT)
                .forEach(col -> pieceList.add(createPiece(Piece.Color.WHITE, Piece.Type.PAWN, new Point(col, rowIndex))));
        return pieceList;
    }
}
